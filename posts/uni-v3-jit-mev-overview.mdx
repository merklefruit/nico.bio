...
> In this article I analyze the Uniswap-v3 Just-in-time MEV strategy.
...
<Bookmark
  ...
  desc="intro to JIT liquidity attack by chainsight labs"
/>
...
## A Naive execution pipeline

As a style exercise, here is what I would consider doing to execute this strategy in the simplest way possible:

1. Build a bot that can quickly find JIT opportunities by looking at mempool & pool reserves
2. Try to write down and log the potential profit of a contract executing these strategies
3. Assess if it’s worth to spend time and effort to continue with the idea
4. If it’s doable, design an “unsafe and simple” contract to execute the JIT attack
5. Deploy on an EVM chain with public mempool (possibly with low fees to test)
6. Iterate/learn/improve
